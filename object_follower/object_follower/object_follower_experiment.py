#!/usr/bin/env python3
"""
object_follower_node_full.py - FIXED VERSION
...
[FIXES APPLIED]:
 1. Added exploration_locked flag to prevent detection_callback from interfering with waypoint navigation
 2. State guards in detection_callback to respect EXPLORING state
 3. Improved goal cancellation logic with proper state transitions
 4. Enhanced logging to track Nav2 goal lifecycle
 5. Debouncing for detection-triggered actions
"""
...
WAYPOINT_MARGIN = 2.5  # Increased from 1.0 to prevent out-of-bounds during navigation
...
# [FIX NOTE]: Increased threshold to delay TRACKING switch until object is closer
# This prevents premature cancellation of exploration waypoint goals
TRACKING_SWITCH_RATIO = 0.08    # Reduced to 0.08 for extremely responsive tracking
TRACKER_TYPE = 'CENTROID'       # 'CENTROID' fallback for systems without full OpenCV contrib
TRACKING_LOST_TIMEOUT = 1.5     # sec until tracker considered lost

# [FIX] Missing Attribute initialization
# Class to handle basic tracking when OpenCV contrib missing
class SimpleCentroidTracker:
    ...
    def update(self, image):
        # This simple tracker relies 100% on the periodic YOLO updates
        # It just holds the last known position until YOLO updates it
        # This is "dead reckoning" between frames
...
EXPLORING = "EXPLORING"
SEARCHING = "SEARCHING"
APPROACHING = "APPROACHING"  # [NEW] Hybrid state: Nav2 approach to visible object
TRACKING = "TRACKING"
STOPPED = "STOPPED"
...
class ObjectFollower(Node):
    def __init__(self):
        ...
        # [FIX NOTE]: Added exploration_locked flag to prevent detection_callback 
        # from interfering with waypoint navigation
        self.exploration_locked = False  # When True, detection_callback won't send approach goals
        self.last_waypoint_send_time = 0.0  # Track when we last sent a waypoint goal
        self._nav_lock = threading.Lock()   # [FIX] Initialize lock for thread-safe Nav2 canceling
        self._warned_no_twist_mux = False   # [FIX] Initialize warning flag

        # [FIX] Add reinit interval for Hybrid Tracking
        self.yolo_reinit_interval = 5  # Reduced to 5 for Centroid mode (needs frequent updates)
        ...
        # QoS for sensors (Best Effort to match simulation/hardware)
        ...
        self.create_subscription(CameraInfo, '/camera/camera_info', self.camera_info_callback, qos_sensor)
        ...
        self.get_logger().info("Object follower node (full, CSRT) started - FIXED VERSION")
        ...
        # [RESEARCH] Switch between Random (A) and Systematic (B) search
        self.declare_parameter('use_random_search', False)
        self.use_random_search = self.get_parameter('use_random_search').get_parameter_value().bool_value
        self.get_logger().info(f"Research Mode: {'RANDOM Search (Algo A)' if self.use_random_search else 'SYSTEMATIC Search (Algo B)'}")
    ...
    def detection_callback(self, msg: Yolov8Inference):
        """
        [FIX NOTE]: CRITICAL CHANGES HERE
        
        This callback now respects the EXPLORING state and won't interfere with waypoint navigation.
        Key improvements:
        1. Check exploration_locked flag before sending approach goals
        2. Only switch to TRACKING when object is very close (increased threshold)
        3. Don't cancel exploration goals unless absolutely necessary
        4. Add logging to track decision points
        """
        ...
        # [FIX NOTE]: Log detection with ratio for debugging
        self.get_logger().debug(f"Detection: {best_det.class_name}, ratio={ratio:.3f}, state={self.state}, exploration_locked={self.exploration_locked}")

        # [CRITICAL FIX]: If using CENTROID tracker, we MUST update it with every YOLO detection
        # otherwise it will never move from the first initialized position
        if self.state == TRACKING and self.tracker and self.tracker_type == 'CENTROID':
            # convert detection to x,y,w,h
            left, top, right, bottom = self._scale_detection_to_camera(best_det)
            x, y, w, h = left, top, right - left, bottom - top
            self.tracker.init(self.camera_image, (x, y, w, h))
            self.yolo_reinit_counter = 0 # reset reinit counter since we just updated
            # also update last_detection_time to prevent timeouts
            self.last_detection_time = time.time()

        # Near (visual) stop - only if already in TRACKING or very close
        if ratio >= DETECTION_STOP_THRESHOLD:
            # [FIX NOTE]: Only stop if we're already tracking or in STOPPED state
            # Don't interrupt exploration for distant objects
            if self.state in [TRACKING, STOPPED]:
                self.state = STOPPED
        ...
        # CASE 1: Object CLOSE (> 12%) -> Pure Visual Tracking
        if ratio >= self.tracking_switch_ratio:
            if self.state in [SEARCHING, EXPLORING, APPROACHING]:
                self.get_logger().info(f"Object close (ratio={ratio:.3f}), switching to TRACKING")
                
                # Cancel any Nav2 goals (Approaching or Exploring)
                with self._nav_lock:
                    try:
                        self.cancel_current_goal()
                        self.exploration_locked = False
                    except Exception:
                        pass
                ...
                self.search_start_time = None
                self.publish_status(f"Object '{best_det.class_name}' close! TRACKING (Visual Servoing).")
                ...
                if self.camera_image is not None:
                    try:
                        left, top, right, bottom = self._scale_detection_to_camera(best_det)
                        x, y, w, h = left, top, right - left, bottom - top
                        # Re-init tracker based on type
                        if self.tracker_type == 'CENTROID':
                            self.tracker = self._create_tracker('CENTROID')
                        else:
                            self.tracker = self._create_tracker('CSRT') # Fallback to CSRT/KCF if configured
        ...
        # CASE 2: Object FAR (> 5%) -> Nav2 Approach
        elif ratio >= APPROACH_SWITCH_RATIO:
            # If in EXPLORING or SEARCHING, switch to APPROACHING
            if self.state in [EXPLORING, SEARCHING]:
                self.get_logger().info(f"Object detected (ratio={ratio:.3f}), switching to APPROACHING (Nav2)")
                
                with self._nav_lock:
                    try:
                        self.cancel_current_goal() # Cancel exploration goal
                        self.exploration_locked = False
                    except Exception:
                        pass
                
                self.state = APPROACHING
                self.publish_status(f"Object '{best_det.class_name}' detected. APPROACHING (Nav2).")
                self.send_nav_goal_to_map_point(best_det)
                self.last_nav_goal_time = time.time()
                return
            
            # If already APPROACHING, update goal periodically
            elif self.state == APPROACHING:
                now = time.time()
                if now - self.last_nav_goal_time > 2.0:
                    self.send_nav_goal_to_map_point(best_det)
                    self.last_nav_goal_time = now
                return

        # CASE 3: Object too small -> Ignore
        else:
            return
    ...
    def send_nav_goal(self, x, y, yaw=0.0):
        """
        [FIX NOTE]: Enhanced logging to track goal lifecycle
        """
        ...
        should_send = self.should_send_goal(x, y, yaw)
        if not should_send:
            now = time.time()
            reasons = []
            if (now - self.last_goal_time) < MIN_GOAL_PERIOD:
                reasons.append(f"MIN_GOAL_PERIOD ({now - self.last_goal_time:.2f}s < {MIN_GOAL_PERIOD}s)")
            if (now - self.last_nav_result_time) < NAV_RESULT_COOLDOWN:
                reasons.append(f"NAV_RESULT_COOLDOWN ({now - self.last_nav_result_time:.2f}s < {NAV_RESULT_COOLDOWN}s)")
            if self.goal_sent and self.last_nav_goal:
                dx = x - self.last_nav_goal[0]
                dy = y - self.last_nav_goal[1]
                dist = math.hypot(dx, dy)
                if dist <= MIN_GOAL_UPDATE:
                    reasons.append(f"MIN_GOAL_UPDATE (dist={dist:.3f}m <= {MIN_GOAL_UPDATE}m)")
            
            self.get_logger().info(f"Rate-limited: not sending goal to ({x:.2f},{y:.2f}). Reasons: {', '.join(reasons)}")
            return False
        ...
        try:
            ...
            # [FIX NOTE]: Enhanced logging
            self.get_logger().info(f"[NAV2 SEND] Goal #{self.nav_goals_sent_count} to ({x:.2f},{y:.2f}) | State: {self.state}")
            return True
        except Exception as e:
    ...
    def start_scanning(self):
        """
        Rotate in place 360° to 'scan' the surroundings after reaching a waypoint.
        This version is simplified - timer_callback handles the actual rotation.
        """
        # Get current time (use ROS time for sim compatibility)
        try:
            now = float(self.get_clock().now().nanoseconds) / 1e9
        except Exception:
            now = time.time()

        # [REMOVED] Safety check for goal_sent - we call this explicitly after clearing goal state
        # if getattr(self, 'goal_sent', False):
        #     self.get_logger().warn("start_scanning blocked: nav goal still active")
        #     return

        # Initialize scanning state
        self.scanning_start = now
        self.scan_active = True
        self.scan_duration = 6.0  # seconds for full rotation (360°)
        self.scan_speed = 0.5     # rad/s (about 30°/s)
        
        self.publish_status("Scanning at waypoint (rotating 360°)")
        self.get_logger().info(f"[SCAN] Starting 360° rotation (will take {self.scan_duration}s)")
    ...
    def goal_result_callback(self, future):
        """
        [FIX NOTE]: Clear goal state BEFORE calling start_scanning to avoid race condition
        """
        try:
            res = future.result()
            ...
            self.get_logger().info(f"[NAV2 RESULT] Status: {status_str} | State: {self.state} | WP: {self.current_wp}")

            if status == GoalStatus.STATUS_SUCCEEDED:
                if self.state == EXPLORING and self.current_wp is not None:
                    self.get_logger().info("[NAV2] Waypoint reached successfully, scheduling scan")

                    # [CRITICAL FIX] Clear goal state FIRST before calling start_scanning
                    self.goal_sent = False
                    self.goal_handle = None
                    self.last_nav_goal = None
                    self.exploration_locked = False  # Unlock exploration

                    # Stop all motion immediately
                    stop = Twist()
                    stop.linear.x = 0.0
                    stop.angular.z = 0.0
                    for _ in range(3):  # Ensure controller sees zeros
                        try:
                            self.cmd_vel_pub.publish(stop)
                        except Exception:
                            pass

                    # Now start_scanning will not be blocked
                    try:
                        self.start_scanning()
                    except Exception as e:
                        self.get_logger().error(f"start_scanning failed: {e}")
                        # Fallback: just go to next waypoint
                        self.goto_next_waypoint()
                    
                    # Return early to skip the finally block (state already cleared)
                    return
                ...
                if self.current_wp and self.amcl_pose:
                    ...
                    d = math.hypot(dx, dy)
                    if d < NEAR_GOAL_EPS:
                        self.get_logger().info(f"[NAV2] Aborted but near waypoint (d={d:.3f}m) -> treating as reached")
                        if self.state == EXPLORING:
                            # Clear goal state before calling start_scanning
                            self.goal_sent = False
                            self.goal_handle = None
                            self.last_nav_goal = None
                            self.exploration_locked = False
                            ...
                            self.current_wp = None
                        return
                self.get_logger().warning(f"[NAV2] Goal ended with {status_str}")
                self.publish_status(f"Navigation ended ({status_str})")
                self.exploration_locked = False  # Unlock on failure too
        except Exception as e:
            self.get_logger().warning(f"Nav2 goal result callback error: {e}")
        finally:
            # Only clear if not already cleared in the success path
            if self.goal_sent:
                self.goal_sent = False
                self.goal_handle = None
                self.last_nav_goal = None
    ...
    def cancel_current_goal(self):
        """
        [FIX NOTE]: Enhanced logging for goal cancellation tracking
        """
        # Request goal cancellation and set a short lock to avoid immediate re-send
        if self.goal_handle:
            try:
                self.get_logger().info(f"[NAV2 CANCEL] Requesting cancellation | State: {self.state}")
                cancel_future = self.goal_handle.cancel_goal_async()
                def _done(fut):
                    try:
                        _res = fut.result()
                        self.get_logger().info("[NAV2 CANCEL] Cancellation processed")
                    except Exception as e:
                        self.get_logger().warning(f"[NAV2 CANCEL] Error: {e}")
    ...
    def goto_next_waypoint(self):
        """
        [FIX NOTE]: Lock exploration when sending waypoint goal to prevent detection interference
        """
        ...
        # [RESEARCH] Check if we should force random search (Algo A)
        # Refresh parameter in case it changed (though usually static)
        # self.use_random_search = self.get_parameter('use_random_search').get_parameter_value().bool_value
        
        force_random = self.use_random_search

        if self.explore_waypoints and not force_random:
            # ALGO B: Systematic (use pre-calculated waypoints)
            ...
            # ALGO A: Random (or fallback if no waypoints)
        ...
        self.publish_status(f"[EXPLORING] Navigating to waypoint ({x:.2f},{y:.2f})")

        # Use ROS (sim) time, not wall time.
        now_secs = float(self.get_clock().now().nanoseconds) / 1e9
        if now_secs < getattr(self, 'nav_cancel_pending_until', 0.0):
            self.get_logger().debug("goto_next_waypoint: waiting for cancel debounce to expire")
            # schedule retry later by returning (timer loop will try again)
            return

        # [FIX NOTE]: Lock exploration to prevent detection_callback from interfering
        self.exploration_locked = True
        self.last_waypoint_send_time = now_secs
        self.get_logger().info("[EXPLORATION LOCKED] Waypoint goal sent, locking exploration")

        sent = self.send_nav_goal(x, y, yaw)
        if not sent:
            self.publish_status("Failed to send waypoint to Nav2 (not ready)")
            self.exploration_locked = False  # Unlock if send failed
    ...
    def start_scanning(self):
        ...
        # Safety checks
        if now < getattr(self, 'next_scan_allowed_time', 0.0):
            self.get_logger().debug("start_scanning blocked: debounce timer active")
            return
    ...
    def publish_twist(self, linear=0.0, angular=0.0):
        ...
        self._publish_tracking_twist(t)

    def rotate_in_place(self):
        self.publish_twist(linear=0.0, angular=ROTATION_SPEED)

    def stop_motion(self):
        """Stop robot motion. Cancel active nav goals only if actually active."""
        try:
            has_goal_handle = getattr(self, 'goal_handle', None) is not None
            goal_sent_flag = bool(getattr(self, 'goal_sent', False))  # defensive

            # Try to detect an active goal: prefer a goal_sent flag if you maintain it.
            if has_goal_handle and goal_sent_flag:
                try:
                    self.get_logger().info("stop_motion: canceling active nav goal")
                    self.cancel_current_goal()
                except Exception as e:
                    self.get_logger().warning(f"stop_motion: cancel failed: {e}")
            else:
                # no active goal -> don't call cancel (avoid messing with nav timing)
                self.get_logger().debug("stop_motion: no active nav goal; skipping cancel")
        except Exception as e:
            self.get_logger().warning(f"stop_motion: unexpected error: {e}")

        # Zero tracking topic (ensures twist_mux receives zero)
        try:
            self._publish_tracking_stop()
        except Exception:
            self.get_logger().debug("stop_motion: _publish_tracking_stop failed or missing")
        ...
        for _ in range(3):
            try:
                ...
                # if publish_twist signature differs, fallback to manual Twist publish
                try:
                    t = Twist()
                    t.linear.x = 0.0
                    t.angular.z = 0.0
    ...
    def _create_tracker(self, tracker_type):
        """
        Create tracker with compatibility for different OpenCV versions.
        Fallback to SimpleCentroidTracker if OpenCV trackers are missing.
        """
        ...
        if tracker is None:
            self.get_logger().error(f"Could not create tracker {tracker_type} - OpenCV version check failed")
            raise RuntimeError(f"OpenCV tracker {tracker_type} not found")
        return tracker
    ...
    def timer_callback(self):
        """
        [FIX NOTE]: Improved EXPLORING state handling
        - Removed premature stop when detection present during exploration
        ...
        if self.state == EXPLORING:
            # Auto-unlock exploration after some time if goal completed or timed out
            if self.exploration_locked and (now - self.last_waypoint_send_time) > 15.0:
                if not self.goal_sent:
                    self.get_logger().info("[EXPLORATION] Auto-unlocking after timeout")
                    self.exploration_locked = False

            # [FIX] Handle scanning in timer_callback for reliability
            if getattr(self, 'scan_active', False):
                scanning_start = getattr(self, 'scanning_start', None)
                if scanning_start is None:
                    # Safety: if scan_active but no start time, reset
                    self.scan_active = False
                    self.get_logger().warn("[SCAN] scan_active but no scanning_start, resetting")
                    return
                    
                elapsed = now - scanning_start
                scan_duration = getattr(self, 'scan_duration', 6.0)
                scan_speed = getattr(self, 'scan_speed', 0.5)
                ...
                if elapsed < scan_duration:
                    ...
                    if int(elapsed) != int(getattr(self, '_last_scan_log', -1)):
                        self._last_scan_log = int(elapsed)
                        self.get_logger().info(f"[SCAN] Progress: {elapsed:.1f}/{scan_duration:.1f}s")
                    return
                else:
                    # Scan complete
                    self.get_logger().info("[SCAN] Completed 360° rotation")
                    self.scan_active = False
                    self.scanning_start = None
                    self._last_scan_log = -1
                    
                    # Stop rotation
                    stop = Twist()
                    stop.linear.x = 0.0
                    stop.angular.z = 0.0
                    for _ in range(3):  # Ensure it stops
                        self._publish_tracking_twist(stop)
                    
                    # Unlock and move to next waypoint
                    self.exploration_locked = False
                    self.get_logger().info("[SCAN] Moving to next waypoint")
                    
                    # [FIX] Reset rate limiting to allow immediate waypoint send
                    self.last_goal_time = 0.0
                    self.last_nav_result_time = now - NAV_RESULT_COOLDOWN - 1.0  # Allow immediate send
                    
                    self.goto_next_waypoint()
                    return

            # If no current waypoint, get the next one
        ...
        # [HYBRID ARCHITECTURE] Handle APPROACHING state
        if self.state == APPROACHING:
            # If object lost for too long -> SEARCHING
            if self.last_detection_time and (now - self.last_detection_time) > 3.0:
                self.get_logger().info("Object lost during APPROACHING -> switching to SEARCHING")
                self.state = SEARCHING
                self.cancel_current_goal()
                self.start_searching()
                return
            # Note: Transition to TRACKING handled in detection_callback
            return
    ...
    def start_searching(self):
        ...
        # [FIX NOTE]: Unlock exploration when switching to searching
        self.exploration_locked = False
